'use strict';
import { createElement as h, render, unmountComponentAtNode } from '../src/neact';
/** @jsx h */
...
describe('svg', () => {
	...
	it('should use attributes for className', () => {
		...
		let root = render(<Demo c="1" />, scratch);
		sinon.spy(root, 'removeAttribute');
		root = render(<Demo />, scratch);
	...
	it('should switch back to HTML for <foreignObject>', () => {
		...
		expect(scratch.getElementsByTagName('a').length).to.equal(1);
	});
