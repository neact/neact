...
describe('refs', () => {
	...
	it('should pass children to ref functions', () => {
		...
		expect(inner).to.have.been.calledOnce.and.calledWith(inst._vNode.dom);
	...
	it('should pass high-order children to ref functions', () => {
		...
		expect(innermost, 'innerMost initial').to.have.been.calledOnce.and.calledWith(innerInst._vNode.dom);

		outer.reset();
		inner.reset();
		innermost.reset();
		root = render(<Outer ref={outer} />, scratch);
		...
		expect(innermost, 'innerMost update').to.have.been.calledOnce.and.calledWith(innerInst._vNode.dom);
	...
	it('should not pass ref into component as a prop', () => {
		...
		expect(Foo.prototype.render).to.have.been.calledWithMatch({ a:'a'}, { }, { });
		expect(Bar).to.have.been.calledWithMatch({ b:'b'}, {});
	});
	...
	it('should only null refs after unmount', () => {
		...
		root = render(<div><TestUnmount /></div>, scratch);
